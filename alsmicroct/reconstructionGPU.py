...
########################
#This script is mostly identical to the non-GPU script except GPU kwargs must be provided in 
#for the recon() function call. 
...
#the accelerated kwargs acts as a switch that can allow non-GPU accelerated methods to be
#used as well, but the default is True 
########################
...
def recon_setup(
    ...
    recon_algorithm='sirt',  # choose from gridrec, fbp, and others in tomopy
...
#					tomo = tomo[:, :, 10:-10]
                    ...
                    if projIgnoreList is not None:
                        for badproj in projIgnoreList:
                            tomo[badproj] = 0
                    if accelerated:
                        rec = tomopy.recon(tomo, anglelist, center=cor+npad, algorithm=recon_algorithm, accelerated = accelerated, pool_size = pool_size, interpolation = interpolation, device = device, ncore = ncore, num_iter = iterations)
                    # need to add lines here to test other configs like "sirt" and "non-accelerated" (remove the filters)    
                    else:
...
def remove_outlier1d(arr, dif, size=3, axis=0, ncore=None, out=None):
    ...
    with cf.ThreadPoolExecutor(ncore) as e:
        ...
        for i in range(ncore):
            ...
            e.submit(snf.median_filter, arr[tuple(slc)], size=filt_size,output=tmp[slc], mode='mirror')
